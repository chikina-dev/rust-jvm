...
use nom::{
    ...
    error::ErrorKind,
    multi::count,
    number::complete::{be_i32, be_u8, be_u16, be_u32},
};

use crate::{
    structure::code::{CODE_BYTES, CodeByte},
    util::{class::parse_constant_pool, hex::hex_utf8},
};
    ...
    pub fn parse<'a>(
        ...
        match name {
            "SourceFile" => {
                ...
                Ok((
                    input,
                    Self::SourceFile(SourceFileAttribute {
            ...
            "SourceDebugExtension" => {
                ...
                Ok((
                    input,
                    Self::SourceDebugExtension(SourceDebugExtensionAttribute {
                        attribute_name_index: index,
                        attribute_length,
            ...
            "LineNumberTable" => {
                ...
                let (input, line_number_table) = count(
                    LineNumberTableEntry::parse,
                    line_number_table_length as usize,
                )
                .parse(input)?;
                Ok((
                    input,
                    Self::LineNumberTable(LineNumberTableAttribute {
                        attribute_name_index: index,
                        attribute_length,
            ...
            "InnerClasses" => {
                ...
                let (input, classes) =
                    count(parse_class_info, number_of_classes as usize).parse(input)?;
                Ok((
                    input,
                    Self::InnerClasses(InnerClassesAttribute {
                        attribute_name_index: index,
                        attribute_length,
                        number_of_classes,
                        classes,
            ...
            "EnclosingMethod" => {
                ...
                Ok((
                    input,
                    Self::EnclosingMethod(EnclosingMethodAttribute {
                        attribute_name_index: index,
                        attribute_length,
                        class_index,
                        method_index,
            ...
            "BootstrapMethods" => {
                ...
                fn parse_bootstrap_method(input: &[u8]) -> IResult<&[u8], BootstrapMethod> {
                    ...
                    let (input, bootstrap_arguments) =
                        count(be_u16, num_bootstrap_arguments as usize).parse(input)?;
                    Ok((
                        input,
                        BootstrapMethod {
                ...
                let (input, bootstrap_methods) =
                    count(parse_bootstrap_method, num_bootstrap_methods as usize).parse(input)?;
                Ok((
                    input,
                    Self::BootstrapMethods(BootstrapMethodsAttribute {
                        attribute_name_index: index,
                        attribute_length,
            ...
            "Module" => {
                ...
                let (input, requires) =
                    count(parse_requires, requires_count as usize).parse(input)?;
                ...
                fn parse_exports(input: &[u8]) -> IResult<&[u8], ModuleExports> {
                    ...
                    let (input, exports_to) =
                        count(be_u16, exports_to_count as usize).parse(input)?;
                    Ok((
                        input,
                        ModuleExports {
                            exports_index,
                ...
                fn parse_provides(input: &[u8]) -> IResult<&[u8], ModuleProvides> {
                    ...
                    let (input, provides_with) =
                        count(be_u16, provides_with_count as usize).parse(input)?;
                    Ok((
                        input,
                        ModuleProvides {
                            provides_index,
                ...
                let (input, provides) =
                    count(parse_provides, provides_count as usize).parse(input)?;
                Ok((
                    input,
                    Self::Module(ModuleAttribute {
                        attribute_name_index: index,
                        attribute_length,
                        module_name_index,
                        module_flags,
            ...
            "ModulePackages" => {
                ...
                Ok((
                    input,
                    Self::ModulePackages(ModulePackagesAttribute {
                        attribute_name_index: index,
                        attribute_length,
                        packages_count,
                        packages,
            ...
            "ModuleMainClass" => {
                ...
                Ok((
                    input,
                    Self::ModuleMainClass(ModuleMainClassAttribute {
                        attribute_name_index: index,
                        attribute_length,
                        main_class_index,
                    }),
            ...
            "NestHost" => {
                ...
                Ok((
                    input,
                    Self::NestHost(NestHostAttribute {
                        attribute_name_index: index,
                        attribute_length,
                        nest_host_index,
                    }),
            ...
            "NestMembers" => {
                ...
                Ok((
                    input,
                    Self::NestMembers(NestMembersAttribute {
                        attribute_name_index: index,
                        attribute_length,
                        number_of_classes,
                        classes,
            ...
            "Record" => {
                ...
                let parse_record_component =
                    |input_inner: &'a [u8]| -> IResult<&'a [u8], RecordComponentInfo> {
                        // ここで 'a を明示
                        ...
                        let parse_record_component_info_attribute = |input_deep: &'a [u8]| -> IResult<
                            &'a [u8],
                            RecordComponentInfoAttribute,
                        > {
                            // ここで 'a を明示
                            ...
                            let (input_deep, attribute) =
                                RecordComponentInfoAttribute::parse(input_deep, &name, idx)?;
                            Ok((input_deep, attribute))
                        };

                        let (input_inner, attributes) = count(
                            parse_record_component_info_attribute,
                ...
                let (input, record_components) =
                    count(parse_record_component, record_components_count as usize).parse(input)?;
                Ok((
                    input,
                    Self::Record(RecordAttribute {
                        attribute_name_index: index,
                        attribute_length,
            ...
            "PermittedSubclasses" => {
                ...
                Ok((
                    input,
                    Self::PermittedSubclasses(PermittedSubclassesAttribute {
                        attribute_name_index: index,
                        attribute_length,
                        number_of_classes,
                        classes,
            ...
            "RuntimeVisibleAnnotations" => {
                ...
                let (input, annotations) =
                    count(Annotation::parse, num_annotations as usize).parse(input)?;
                Ok((
                    input,
                    Self::RuntimeVisibleAnnotations(RuntimeVisibleAnnotationsAttribute {
                        attribute_name_index: index,
                        attribute_length,
                        num_annotations,
                        annotations,
            ...
            "RuntimeInvisibleAnnotations" => {
                ...
                Ok((
                    input,
                    Self::RuntimeInvisibleAnnotations(RuntimeInvisibleAnnotationsAttribute {
                        attribute_name_index: index,
                        attribute_length,
                        num_annotations,
                        annotations,
            ...
            "RuntimeVisibleTypeAnnotations" => {
                ...
                let (input, num_annotations) = be_u16(input)?;
                let (input, annotations) =
                    count(TypeAnnotation::parse, num_annotations as usize).parse(input)?;
                Ok((
                    input,
                    Self::RuntimeVisibleTypeAnnotations(RuntimeVisibleTypeAnnotationsAttribute {
                        attribute_name_index: index,
                        attribute_length,
                        num_annotations,
                        annotations,
            ...
            "Synthetic" => {
                ...
                Ok((
                    input,
                    Self::Synthetic(SyntheticAttribute {
                        attribute_name_index: index,
                        attribute_length,
                    }),
                ))
            ...
            "Deprecated" => {
                ...
                Ok((
                    input,
                    Self::Deprecated(DeprecatedAttribute {
                        attribute_name_index: index,
                        attribute_length,
                    }),
                ))
            ...
            "Signature" => {
                ...
                Ok((
                    input,
                    Self::Signature(SignatureAttribute {
                        attribute_name_index: index,
                        attribute_length,
                        signature_index,
                    }),
    ...
    pub fn parse<'a>(input: &'a [u8], name: &str, index: u16) -> IResult<&'a [u8], Self> {
        match name {
            "ConstantValue" => {
                ...
                Ok((
                    input,
                    Self::ConstantValue(ConstantValueAttribute {
                        attribute_name_index: index,
                        attribute_length,
...
fn parse_code_bytes(input: &[u8], code_length: usize) -> IResult<&[u8], Vec<CodeByte>> {
    ...
    while pc < code_length {
        ...
        let mut code_byte = CODE_BYTES
            .get(&opcode)
            .cloned()
            .unwrap_or_else(|| unknown_code_byte(opcode));
        let operand_len = match opcode {
            0xaa => {
                let padding = parse_switch_padding(opcode_pc);
                let after_padding = remaining.get(padding..).ok_or_else(|| {
                    nom::Err::Error(nom::error::Error::new(remaining, ErrorKind::Eof))
                })?;
            ...
            0xc4 => {
                let modified_opcode = *remaining.first().ok_or_else(|| {
                    nom::Err::Error(nom::error::Error::new(remaining, ErrorKind::Eof))
                })?;
        ...
        match name {
            "Code" => {
                ...
                let (input, exception_table) =
                    count(exception_entry, exception_table_length as usize).parse(input)?;
                let (input, attributes_count) = be_u16(input)?;
                let parse_code_attributes =
                    |input_inner: &'a [u8]| -> IResult<&'a [u8], CodeAttributes> {
                        let parse_code_nested_attribute =
                            |input_deep: &'a [u8]| -> IResult<&'a [u8], CodeNestedAttribute> {
                                let (input_deep, idx) = be_u16(input_deep)?;
                                let name = match constant_pool.get_class(idx) {
                                    Ok(Constant::Utf8 { bytes, .. }) => hex_utf8(bytes),
                                    _ => {
                                ...
                                let (input_deep, attribute) =
                                    CodeNestedAttribute::parse(input_deep, &name, idx)?;
                                Ok((input_deep, attribute))
                            };
                        let (input_inner, attributes) =
                            count(parse_code_nested_attribute, attributes_count as usize)
                                .parse(input_inner)?;
                        Ok((
                            input_inner,
                            CodeAttributes {
            ...
            "Exceptions" => {
                ...
                let (input, exception_index_table) =
                    count(be_u16, number_of_exceptions as usize).parse(input)?;
                Ok((
                    input,
                    Self::Exceptions(ExceptionsAttribute {
                        attribute_name_index: index,
                        attribute_length,
            ...
            "AnnotationDefault" => {
                ...
                Ok((
                    input,
                    Self::AnnotationDefault(AnnotationDefaultAttribute {
                        attribute_name_index: index,
                        attribute_length,
                        default_value,
                    }),
            ...
            "MethodParameters" => {
                ...
                let (input, parameters) =
                    count(parameter, parameters_count as usize).parse(input)?;
                Ok((
                    input,
                    Self::MethodParameters(MethodParametersAttribute {
                        attribute_name_index: index,
                        attribute_length,
                        parameters_count,
                        parameters,
            ...
            "RuntimeVisibleParameterAnnotations" => {
                ...
                let (input, parameter_annotations) =
                    count(parameter_annotations, num_parameters as usize).parse(input)?;
                Ok((
                    input,
    ...
    pub fn parse<'a>(input: &'a [u8], name: &str, index: u16) -> IResult<&'a [u8], Self> {
        match name {
            "LineNumberTable" => {
                ...
                ) -> IResult<&[u8], Vec<LineNumberTableEntry>> {
                ...
                let (input, line_number_table) =
                    parse_line_number_table(input, line_number_table_length)?;
            ...
            "LocalVariableTable" => {
                ...
                let (input, local_variable_table) =
                    parse_local_variable_table(input, local_variable_table_length)?;
            ...
            "LocalVariableTypeTable" => {
                ...
                let (input, local_variable_type_table) =
                    parse_local_variable_type_table(input, local_variable_type_table_length)?;
            ...
            "RuntimeVisibleTypeAnnotations" => {
                ...
                let (input, annotations) =
                    parse_runtime_visible_type_annotations(input, num_annotations)?;
            ...
            "RuntimeInvisibleTypeAnnotations" => {
                ...
                let (input, annotations) =
                    parse_runtime_invisible_type_annotations(input, num_annotations)?;
    ...
    pub fn parse<'a>(input: &'a [u8], name: &str, index: u16) -> IResult<&'a [u8], Self> {
        match name {
            ...
            "RuntimeVisibleAnnotations" => {
                ...
                let (input, annotations) =
                    parse_runtime_visible_annotations(input, num_annotations)?;
            ...
            "RuntimeInvisibleAnnotations" => {
                ...
                let (input, annotations) =
                    parse_runtime_invisible_annotations(input, num_annotations)?;
...
impl StackMapFrame {
    pub fn parse(input: &[u8]) -> IResult<&[u8], Self> {
        ...
        match frame_type {
            ...
            128..=246 => Err(nom::Err::Error(nom::error::Error::new(
                input,
                ErrorKind::Tag,
            ))),
            248..=250 => {
            ...
            252..=254 => {
                ...
                let (input, locals) =
                    VerificationTypeInfo::parse_vec_with_count(input, locals_count)?;
                Ok((
                    input,
            ...
            255 => {
                ...
                let (input, locals) =
                    VerificationTypeInfo::parse_vec_with_count(input, number_of_locals as usize)?;
                let (input, number_of_stack_items) = be_u16(input)?;
                let (input, stack) = VerificationTypeInfo::parse_vec_with_count(
                    input,
                    number_of_stack_items as usize,
                )?;
                Ok((
                    input,
        ...
        match target_type {
            ...
            0x40 | 0x41 => {
                ...
                for _ in 0..table_length {
                    ...
                    local_var_table.push(LocalVarTableEntry {
                        start_pc,
                        length,
                        index,
                    });
    ...
    fn parse(input: &[u8]) -> IResult<&[u8], ElementValue> {
        ...
        match tag {
            ...
            0x5B => {
                ...
                value.value = ElementValueEnum::ArrayValue { num_values, values };
                Ok((input, value))
            }
            _ => Err(nom::Err::Error(nom::error::Error::new(
                input,
...
impl<'a> ClassFileParser {
    ...
    fn parse_interfaces(&self, input: &'a [u8]) -> IResult<&'a [u8], Interfaces> {
        ...
        let (input, interfaces) =
            nom::multi::count(be_u16, interfaces_count as usize).parse(input)?;
    ...
    fn parse_field(&self, input: &'a [u8]) -> IResult<&'a [u8], Field> {
        ...
        let parse_field_info_attribute =
            |input: &'a [u8]| -> IResult<&'a [u8], FieldInfoAttribute> {
                let (input, index) = be_u16(input)?;
        ...
        let (input, attributes) =
            nom::multi::count(parse_field_info_attribute, attributes_count as usize)
                .parse(input)?;
    ...
    fn parse_method(&self, input: &'a [u8]) -> IResult<&'a [u8], Method> {
        ...
        let parse_method_info_attribute =
            |input: &'a [u8]| -> IResult<&'a [u8], MethodInfoAttribute> {
                let (input, index) = be_u16(input)?;
                let name = match self.constant_pool.get_class(index) {
                    Ok(Constant::Utf8 { bytes, .. }) => hex_utf8(bytes),
                    _ => {
                ...
                let (input, attribute) =
                    MethodInfoAttribute::parse(input, &name, index, &self.constant_pool)?;
                Ok((input, attribute))
            };

        let (input, attributes) =
            nom::multi::count(parse_method_info_attribute, attributes_count as usize)
                .parse(input)?;
    ...
    fn parse_methods(&self, input: &'a [u8]) -> IResult<&'a [u8], Methods> {
        ...
        let (input, methods) =
            count(|i| self.parse_method(i), methods_count as usize).parse(input)?;
        Ok((
            input,
            Methods {
                methods_count,
    ...
    fn parse_class_file_attribute(&self, input: &'a [u8]) -> IResult<&'a [u8], ClassFileAttribute> {
        ...
        let (input, attribute) =
            ClassFileAttribute::parse(input, &name, index, &self.constant_pool)?;
        Ok((input, attribute))
    }

    fn parse_class_file_attributes(
        &self,
        input: &'a [u8],
    ) -> IResult<&'a [u8], ClassFileAttributes> {
        let (input, attributes_count) = be_u16(input)?;
        let (input, attributes) = nom::multi::count(
            |i| self.parse_class_file_attribute(i),
            attributes_count as usize,
        )
        .parse(input)?;
        Ok((
