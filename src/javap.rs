use crate::structure::class::{
    ClassFile, ClassFileAttribute, CodeAttribute, Constant, MethodInfoAttribute,
};
use crate::util::class::{
    class_access_flags, constant_pool_viewer, field_access_flags, java_version_name,
    method_access_flags,
};
...
pub fn javap_viewer(class_file: ClassFile) {
    ...
    println!(
        "このファイルは{}バージョンのJavaでコンパイルされました",
        java_version_name(class_file.header.major)
    );
    ...
    println!(
        "\nAccess Flags: {}",
        class_access_flags(class_file.access_flags)
    );
    println!(
        "This Class: #{}: {:?}",
        class_file.this_class,
        class_file.constant_pool.constants[class_file.this_class as usize - 1]
    );
    println!(
        "Super Class: #{}: {:?}",
        class_file.super_class,
        class_file.constant_pool.constants[class_file.super_class as usize - 1]
    );

    println!(
        "\nInterfaces count: {}",
        class_file.interfaces.interfaces_count
    );
    for (_, interface) in class_file.interfaces.interfaces.iter().enumerate() {
        println!(
            "Interface #{}: {:?}",
            interface,
            class_file.constant_pool.constants[*interface as usize - 1]
        );
    }
    ...
    for (i, field) in class_file.fields.fields.iter().enumerate() {
        ...
        println!(
            "Name: #{}: {:?}",
            field.name_index,
            class_file.constant_pool.constants[field.name_index as usize - 1]
        );
        println!(
            "Descriptor: #{}: {:?}",
            field.descriptor_index,
            class_file.constant_pool.constants[field.descriptor_index as usize - 1]
        );
    }
    ...
    for (i, method) in class_file.methods.methods.iter().enumerate() {
        ...
        let method_descriptor =
            match &class_file.constant_pool.constants[method.descriptor_index as usize - 1] {
                Constant::Utf8 { bytes, .. } => hex_utf8(&bytes),
                _ => format!("#{}", method.descriptor_index),
            };
        println!(
            "Descriptor: #{}: {:?}",
            method.descriptor_index, method_descriptor
        );
        ...
        if !method.attributes.attributes.is_empty() {
            ...
            for attr in &method.attributes.attributes {
                match attr {
                    MethodInfoAttribute::Code(CodeAttribute {
                        code,
                        max_stack,
                        max_locals,
                        ..
    ...
    println!(
        ...
        class_file.attributes.attributes_count
    );
    for attr in &class_file.attributes.attributes {
        match attr {
            ClassFileAttribute::SourceFile(source_file) => {
                let source_file_name = match &class_file.constant_pool.constants
                    [source_file.source_file_index as usize - 1]
                {
                    Constant::Utf8 { bytes, .. } => hex_utf8(&bytes),
