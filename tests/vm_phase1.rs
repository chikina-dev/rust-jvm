...
fn compile_fixture(class_name: &str) -> Option<PathBuf> {
    ...
    let output = match output {
        ...
        Err(error) if error.kind() == std::io::ErrorKind::NotFound => {
            eprintln!("skipping VM phase1 test because javac is not installed");
            return None;
        }
...
#[test]
fn invokes_static_int_main_from_parsed_class_file() {
    let Some(class_path) = compile_fixture("Phase1IntMain") else {
        return;
    };
    ...
    assert_eq!(
        vm.invoke_static("Phase1IntMain", "main", "()I", vec![]),
        Ok(Some(Value::Int(3)))
    );
}
