...
fn compile_fixture(class_name: &str) -> Option<PathBuf> {
    ...
    let output = match output {
        ...
        Err(error) if error.kind() == std::io::ErrorKind::NotFound => {
            eprintln!("skipping VM phase4 test because javac is not installed");
            return None;
        }
...
#[test]
fn invokes_constructor_and_reads_instance_field_from_parsed_class_file() {
    let Some(class_path) = compile_fixture("Phase4ObjectMain") else {
        return;
    };
    ...
    assert_eq!(
        vm.invoke_static("Phase4ObjectMain", "main", "()I", vec![]),
        Ok(Some(Value::Int(9)))
    );
}
