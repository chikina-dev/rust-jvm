...
fn compile_fixture(class_name: &str) -> Option<PathBuf> {
    ...
    let output = match output {
        ...
        Err(error) if error.kind() == std::io::ErrorKind::NotFound => {
            eprintln!("skipping VM phase6 test because javac is not installed");
            return None;
        }
...
#[test]
fn routes_system_out_println_to_vm_stdout_buffer() {
    let Some(class_path) = compile_fixture("Phase6PrintlnMain") else {
        return;
    };
    ...
    assert_eq!(
        vm.invoke_static("Phase6PrintlnMain", "main", "()V", vec![]),
        Ok(None)
    );
    assert_eq!(
        vm.memory.stdout,
