        ...
        return Some(
            dirs.into_iter()
                .map(|dir| {
                    ...
                    CompiledClass {
                        bytes: fs::read(class_path)
                            .expect("failed to read precompiled class fixture"),
                        dir,
                    }
                })
                .collect(),
...
fn precompiled_classes_dirs() -> Option<Vec<PathBuf>> {
        ...
        .map(PathBuf::from)
    {
        ...
        assert!(
            !dirs.is_empty(),
            "precompiled fixture root must contain class directories"
        );
        return Some(dirs);
    }
...
fn method_code_names(class_file: &ClassFile, method_name: &str) -> Vec<&'static str> {
    class_file
        ...
        .find_map(|attribute| match attribute {
            MethodInfoAttribute::Code(code) => {
                Some(code.code.iter().map(|byte| byte.name).collect())
            }
            _ => None,
        })
...
fn for_each_fixture_class(class_name: &str, mut check: impl FnMut(&CompiledClass)) {
    ...
    assert!(
        !compiled_classes.is_empty(),
        "at least one fixture class should be available"
    );
...
fn e2e_parse_constants_switches_and_annotations() {
    for_each_fixture_class("EdgeCases", |compiled| {
        ...
        assert!(class_file.attributes.attributes.iter().any(|attribute| {
            matches!(
                attribute,
                ClassFileAttribute::RuntimeVisibleTypeAnnotations(_)
            )
        }));
    });
}
    ...
    for_each_fixture_class("AdvancedFeatures", |compiled| {
        ...
        assert!(class_file.methods.methods.iter().any(|method| {
            method
                ...
                .attributes
                .iter()
                .any(|attribute| matches!(attribute, MethodInfoAttribute::Exceptions(_)))
        }));
