...
fn compile_fixture(class_name: &str) -> Option<PathBuf> {
    ...
    let output = match output {
        ...
        Err(error) if error.kind() == std::io::ErrorKind::NotFound => {
            eprintln!("skipping VM phase5 test because javac is not installed");
            return None;
        }
...
#[test]
fn creates_and_reads_int_array_from_parsed_class_file() {
    let Some(class_path) = compile_fixture("Phase5ArrayMain") else {
        return;
    };
    ...
    assert_eq!(
        vm.invoke_static("Phase5ArrayMain", "main", "()I", vec![]),
