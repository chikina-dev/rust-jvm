...
fn compile_fixture(class_name: &str) -> Option<PathBuf> {
    ...
    let output = match output {
        ...
        Err(error) if error.kind() == std::io::ErrorKind::NotFound => {
            eprintln!("skipping VM phase2 test because javac is not installed");
            return None;
        }
...
#[test]
fn invokes_static_loop_main_from_parsed_class_file() {
    let Some(class_path) = compile_fixture("Phase2LoopMain") else {
        return;
    };
    ...
    assert_eq!(
        vm.invoke_static("Phase2LoopMain", "main", "()I", vec![]),
