...
fn compile_fixture(class_name: &str) -> Option<PathBuf> {
    ...
    let output = match output {
        ...
        Err(error) if error.kind() == std::io::ErrorKind::NotFound => {
            eprintln!("skipping VM phase8 test because javac is not installed");
            return None;
        }
...
#[test]
fn creates_stores_and_reads_reference_array_from_parsed_class_file() {
    let Some(class_path) = compile_fixture("Phase8ReferenceArrayMain") else {
        return;
    };
