use rust_jvm::{
    ...
    vm::{class::ClassState, value::Value, vm::Vm},
};
...
fn compile_fixture(class_name: &str) -> Option<PathBuf> {
    ...
    let output = match output {
        ...
        Err(error) if error.kind() == std::io::ErrorKind::NotFound => {
            eprintln!("skipping VM phase11 test because javac is not installed");
            return None;
        }
...
#[test]
fn runs_clinit_before_static_main_and_reads_static_field() {
    let Some(class_path) = compile_fixture("Phase11StaticInitMain") else {
        return;
    };
    ...
    assert_eq!(
        vm.invoke_static("Phase11StaticInitMain", "main", "()I", vec![]),
    ...
    let class = vm
        ...
        .method_area
        .class(class_id)
        .expect("runtime class should exist");
    assert_eq!(class.state, ClassState::Initialized);
}
